...
var isFiltering = 0; // 필터 검색 여부를 저장할 변수. 기본값은 0으로 전체상품목록을 의미
...
function onClickFilter() {
    ...
    // 필터링된 상품 개수를 기준으로 페이지네이션을 다시 렌더링합니다.
    renderPagination(filteredProducts.length, pageSize);

    // 필터링된 상품 목록을 전역 변수 productList에 업데이트합니다.
    productList = filteredProducts;

    // 첫 번째 페이지로 이동
    goToPage(1);

    // 필터링 여부를 나타내는 변수를 업데이트합니다.
    isFiltering = 1;
}
...
function renderFilterPagination(filteredProductsCount, pageSize) {
    var totalPages = Math.ceil(filteredProductsCount / pageSize); // 필터링된 상품목록을 pageSize로 나눈 후 올림하여 총 페이지 수를 계산합니다.
