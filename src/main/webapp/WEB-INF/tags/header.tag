...
<body>
    ...
    <div class="collapse navbar-collapse offset" id="navbarSupportedContent">
        <ul class="nav navbar-nav menu_nav justify-content-center">
            <li class="nav-item"><a class="nav-link" href="productList" onmouseover="showDropdownMenu()">상품목록</a>
                <div class="dropdown-menu popover" id="productDropdownMenu" onmouseleave="hideDropdownMenu()">
    ...
    <ul class="nav-shop justify-content-center justify-content-md-end">
        ...
        <c:if test="${ empty sessionMid || ( grade != 2 && not empty sessionMid)}">
            <li class="nav-item"><a href="cart"><i class="ti-shopping-cart"></i></a></li>
        </c:if>
        <c:if test="${not empty sessionMid && grade == 2}">
            <li class="nav-item"><a href="adminDashboard"><i class="ti-user"></i></a></li>
        </c:if>
        <c:if test="${not empty sessionMid}">
            <li class="nav-item"><a class="button button-header" href="logout" id="logout">로그아웃</a></li>
            <li class="nav-item"><a class="button button-header" href="mypage" id="mypage">마이페이지</a></li>
        </c:if>
        <c:if test="${empty sessionMid}">
            <li class="nav-item"><a class="button button-header" href="login" id="login">로그인</a></li>
            <li class="nav-item"><a class="button button-header" href="register" id="register">회원가입</a></li>
        </c:if>
    </ul>
</div>
