	...
	<c:if test="${fn:length(rdatas) > 0}">							<!-- 리뷰가 있는 경우 -->
		<c:forEach var='data' items='${rdatas}'> <!-- 리뷰가 있을 경우 각 리뷰를 돌면서 내용을 표시함 -->
			...
			<!-- 구해한 상품 주문번호, 이미지, 이름, 작성일  -->
				<div class="comment-form"><a href="productDetail?productPK=${data.productPK}"><img style="max-width: 200px; max-height: 200px;"
					src="/resources/${data.productImg}" alt="상품이미지"><br><br>
					<h3 style="text-align: center; margin-bottom: 20px;">${data.productName}</h3></a> 
					<h4 style="text-align: right;">작성일: ${data.reviewRegdate}</h4>
					<form>
						...
						<c:if test="${data.reviewImg != null}">
							...
							class="img-fluid" src="/resources/${data.reviewImg}" alt=""> <!-- 업로드한 이미지 -->
							</div>
						</c:if>
						</div>
						<input type="hidden" name="reviewScope" id="scope" value="${data.reviewScope}"> 
						<star:star id="${data.buyProductPK}" defaultRating="${data.reviewScope}" />		<!-- 기존에 별점이 입력 되있었으면 그 별점값을 불러옴 -->
						<div class="form-group">
							<textarea class="form-control mb-10" rows="5" name="message"
								placeholder="리뷰 내용" onfocus="this.placeholder = ''"
								onblur="this.placeholder = 'Messege'" disabled>${data.reviewContent}
