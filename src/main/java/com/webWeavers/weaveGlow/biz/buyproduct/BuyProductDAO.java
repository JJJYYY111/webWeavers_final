...
public class BuyProductDAO {
	...
	private static final String INSERT = "INSERT INTO BUYPRODUCT (PRODUCT_PK,CART_CNT,SERIAL_PK)VALUES(?,?,(SELECT MAX(SERIAL_PK) FROM SERIAL))";
	...
	public boolean insert(BuyProductDTO buyProductDTO) {
		int result = jdbcTemplate.update(INSERT, buyProductDTO.getProductPK(), buyProductDTO.getCartCnt());
		if(result<=0) {
			return false;
		}
		return true;
