...
public class ProductDAO {
	...
	private static String selectAllMainPageProductList(String sortType) {
		String query = "WITH P_BUYCNT AS (\r\n" // 상품별 총 구매수량 정보 CTE --> 인기순 정렬을 하기위함
				...
				+ " DESC, P.PRODUCT_PK ASC LIMIT 8"; // sortType - 정렬조건
	...
	private static final String SELECTONE_UPDATE = "SELECT\r\n"
			...
			+ "WHERE PRODUCT_PK = ?";
	// Admin_상품등록페이지 (Product Insert 후 --> Categorization Insert 할 때 해당 ProductPK
	// 필요)
...
//	private static final String DELETE = "";
	...
	// 3시간별 비교 매출 총합
	private static final String SELECTALL_TODAYSALES_THREEHOUR = " SELECT TOTAL.TEMP, SUM(PRODUCT_PRICE * BUYPRODUCT_CNT) AS SALES_TOTAL	"
			...
			+ "JOIN SERIAL S ON S.SERIAL_PK = B.SERIAL_PK " + "WHERE DATE(S.SERIAL_REGDATE) = CURDATE()) AS TOTAL " // 구매시간을
																													// 기준으로
																													// 시간을
																													// 계산
																													// =
	...
	// 전날 3시간별 비교 매출총합
	private static final String SELECTALL_PVDAYSALES_THREEHOUR = "SELECT PV_TOTAL.TEMP, SUM(PRODUCT_PRICE * BUYPRODUCT_CNT) AS SALES_TOTAL "
			+ "FROM (SELECT P.PRODUCT_PK, P.PRODUCT_PRICE, B.BUYPRODUCT_CNT, (FLOOR(DATE_FORMAT(S.SERIAL_REGDATE, '%H') / 3) * 3) AS TEMP " // 시간을
																																			// 3시간
																																			// 간격으로
																																			// 그룹화하여
																																			// TEMP열
			...
			+ "WHERE DATE(S.SERIAL_REGDATE) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)) AS PV_TOTAL " // 구매시간을 기준으로 시간을 계산 =
																									// 하루 전 날짜만
			+ "GROUP BY TEMP ";

	// 월별 매출 차트
	private static final String SELECTALL_MONTHLY_SALES = "SELECT DATE_FORMAT(S.SERIAL_REGDATE, '%Y-%m') AS MONTH, "
			+ "SUM(PRODUCT_PRICE * BUYPRODUCT_CNT) AS SALES_TOTAL " + "FROM PRODUCT P "
	...
	private static final String SELECTALL_CATEGORY_SALES = "SELECT S.CATEGORY_PK, "
			...
			+ "JOIN SUBCATEGORY S ON TEMP2.TEMP = S.SUBCATEGORY_PK " + "GROUP BY S.CATEGORY_PK ";
	...
	// 이번 달 총 매출
	private static final String SELECTONE_SALES_THISMONTH = "SELECT SUM(B.BUYPRODUCT_CNT * P.PRODUCT_PRICE) AS MONTH_TOTAL " // 현재월의
																																// 총매출
			+ "FROM BUYPRODUCT B " + "JOIN PRODUCT P ON B.PRODUCT_PK = P.PRODUCT_PK " // BUYPRODUCT 테이블과 PRODUCT 테이블을
																						// PRODUCT_PK를 기준으로 조인
	...
	// 오늘 총 매출
	private static final String SELECTONE_TODAYSALES_TOTAL = "SELECT SUM(B.BUYPRODUCT_CNT * P.PRODUCT_PRICE) AS TOTAL "
			+ "FROM BUYPRODUCT B 	" + "JOIN PRODUCT P ON B.PRODUCT_PK = P.PRODUCT_PK " // BUYPRODUCT 테이블과 PRODUCT 테이블을
																							// PRODUCT_PK를 기준으로 조인
			+ "JOIN SERIAL S ON S.SERIAL_PK = B.SERIAL_PK " // BUYPRODUCT 테이블과 SERIAL 테이블을 SERIAL_PK를 기준으로 조인
	...
	// 어제 총 매출 --
	private static final String SELECTONE_PVDAYSALES_TOTAL = "SELECT SUM(B.BUYPRODUCT_CNT * P.PRODUCT_PRICE) AS PV_TOTAL "
			+ "FROM BUYPRODUCT B " + "JOIN PRODUCT P ON B.PRODUCT_PK = P.PRODUCT_PK " // BUYPRODUCT 테이블과 PRODUCT 테이블을
																						// PRODUCT_PK를 기준으로 조인
			+ "JOIN SERIAL S ON S.SERIAL_PK = B.SERIAL_PK " // BUYPRODUCT 테이블과 SERIAL 테이블을 SERIAL_PK를 기준으로 조인
	...
	public List<ProductDTO> selectAll(ProductDTO productDTO) {
		...
		try {
			...
			if (productDTO.getSearchCondition().equals("userMain")) { // 쿼리반환함수 인자 : DTO를 줘도되지만 정렬조건만 있어도돼서 개별로줌
				return jdbcTemplate.query(selectAllMainPageProductList(productDTO.getSortType()), args1,
						new ProductListUserRowMapper());
			}
			...
			// Admin_전일대비매출페이지 (당일 매출 Top10 상품목록)
				...
				return jdbcTemplate.query(selectAllDailyAndMonthlySales("adminDailySales"),
						new ProductTop10SalesRowMapper());
			}
			// Admin_월별매출페이지 (당월 매출 Top10 상품목록)
			else if (productDTO.getSearchCondition().equals("adminMonthlySales")) {
				return jdbcTemplate.query(selectAllDailyAndMonthlySales("adminMonthlySales"),
						new ProductTop10SalesRowMapper());
			}
	...
	public ProductDTO selectOne(ProductDTO productDTO) {
		...
		try {
			...
			// Admin_판매중인 상품 총 갯수
				...
				return jdbcTemplate.queryForObject(SELECTONE_ADMIN_TOTALPRODUCTNUM,
						new ProductSalesTotalNumAdminRowMapper());
			}
			// Admin_오늘 총 매출
			else if (productDTO.getSearchCondition().equals("adminTodaySales")) {
				return jdbcTemplate.queryForObject(SELECTONE_TODAYSALES_TOTAL,
						new ProductSalesTotalTodayAdminRowMapper());
			}
			// Admin_어제 총 매출
			else if (productDTO.getSearchCondition().equals("adminPvdaySales")) {
				return jdbcTemplate.queryForObject(SELECTONE_PVDAYSALES_TOTAL,
						new ProductSalesTotalPvdayAdminRowMapper());
			}
			// Admin_이번달 총 매출
			else if (productDTO.getSearchCondition().equals("adminSalesThisMonth")) {
				return jdbcTemplate.queryForObject(SELECTONE_SALES_THISMONTH,
						new ProductSalesThisMonthAdminRowMapper());
			}
	...
	public boolean update(ProductDTO productDTO) {
		...
		try {
			...
			// 판매된 상품수만큼 재고수량 수정
				...
				result = jdbcTemplate.update(UPDATE_BYSELLING, productDTO.getProductQuantity(),
						productDTO.getProductPK());
			}
			if (result <= 0) {
				return false;
			}
